...
export default function Explore() {
  ...
  const {
    ...
    [`${process.env.NEXT_PUBLIC_BASE_PATH}/api/datasets.json`],
    fetchBatch
  );
  const datasets = datasetsResponse ? datasetsResponse[0] : [];
  const schemaQueries = datasets.map(
    ({ name, forgedb_versions }) =>
      `${process.env.NEXT_PUBLIC_BASE_PATH}/api/${name}/${forgedb_versions[0].version}/schema.json`
  );
  const tableQueries = datasets.map(
    ({ name, forgedb_versions }) =>
      `${process.env.NEXT_PUBLIC_BASE_PATH}/api/${name}/${forgedb_versions[0].version}/${rsid}.json`
  );
  const {
    data: schemaData,
    error: schemaError,
    isLoading: schemaLoading,
  } = useSWR(schemaQueries, fetchBatch);
  const {
    data: tableData,
    error: tableError,
    isLoading: tableLoading,
  } = useSWR(tableQueries, fetchBatch);
  ...
  const data = datasets?.map(({ name, forgedb_versions }, index) => ({
    ...
    table: tableData?.[index]?.data?.filter(
      getRowFilter(search, schemaData?.[index])
    ),
  }));
  const forgeDbScore = getForgeDbScore(data);
  const closestGene = data?.find((d) => d.name === "closest_gene")
    ?.originalTable?.[0];
          ...
          <div className="col">
            <div className="d-flex flex-wrap align-items-baseline py-3">
              <h1 className="fs-1 text-light fw-light me-2">
                FORGE<small className="fw-normal text-warning">db</small>{" "}
                Summary for
              </h1>

              <form
                action={`${process.env.NEXT_PUBLIC_BASE_PATH}/explore`}
                className="mb-2"
              >
                <div className="input-group border-bottom border-white border-2">
                  <input
                    className="form-control bg-transparent border-0 shadow-0 no-clear-control text-light placeholder-light fw-light ps-0 fs-1"
                    type="search"
                    placeholder="rsid"
                    defaultValue={rsid}
                    aria-label="Enter RSID"
                    name="rsid"
                    pattern="^rs\d+"
                    required
                    style={{ width: "200px" }}
                  />
                  <button
                    className="btn btn-outline-secondary bg-transparent border-0 text-light fs-3"
                    type="submit"
                  >
            ...
            <div className="col">
              {!rsid && !isLoading && (
                <h1 className="fs-2 fw-light">
                  Please enter an RSID to view summary-level data.
                </h1>
              )}
              {rsid && isLoading && (
                <h1 className="fs-2 fw-light">Loading results</h1>
              )}
              {rsid && hasNoData && !isLoading && (
                <h1 className="fs-2 fw-light">
                  No results were found for {rsid}.
                </h1>
              )}
              {rsid && !hasNoData && !isLoading && (
                <>
                  <div className="mb-3">
                    <label className="form-label" htmlFor="search">
                      Tissue/Cell Type/Annotation Filter
                    </label>
                    <input
                  ...
                  <h2 className="fs-5 fw-semibold mb-1 d-flex align-items-baseline justify-content-between">
                    <span>
                      Summary and number of annotations for {rsid}{" "}
                      <Link href="/about">(FORGEdb score={forgeDbScore})</Link>
                    </span>
                    {closestGene && (
                      <a
                        className="small"
                        target="_blank"
                        href={`https://genome.ucsc.edu/cgi-bin/hgTracks?db=hg19&position=${closestGene.chr}%3A${closestGene.start}-${closestGene.start}`}
                      >
                    ...
                    ?.map(({ name, schema, table }, index) => (
                      <div key={index}>
                        <h2
                          ...
                          id={name}
                          dangerouslySetInnerHTML={{ __html: schema.title }}
                        />
                        ...
                        >
                          <table
                            className="table table-sm table-striped table-hover shadow-lg border"
                            tabIndex={0}
                          >
                            ...
                            <tbody>
                              ...
                              {!table?.length && (
                                <tr>
                                  ...
                                  >
                                    Variant does not overlap any regions from
                                    this dataset.
                                  </td>
                                </tr>
                              )}
